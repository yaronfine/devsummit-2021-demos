...
export default class CustomLayerView extends BaseLayerViewGL2D {  
  ...
  private _vaoExt: any;
  private _bufferData: ArrayBuffer;
  ...
  // --------------------------------------------------------------------------
  attach(): void {
    workers.open("app/Processor", { client: this, strategy: "dedicated" })
      .then((connection) => { 
    ...
    customLayer.on("connect", () => this._connect());
    customLayer.on("disconnect", () => this._disconnect());

    this.view.watch("extent", (value: Extent) => { 
      const connection = this._connection;
      const viewpoint = this.view.viewpoint.targetGeometry.toJSON();
      const visibleExtent = this.view.extent.toJSON();
      
      if (connection) { 
        connection.invoke("setViewState", { 
          viewpoint,
          visibleExtent
        });
      }
  ...
  setData(params: { data: ArrayBuffer, bufferLength: number, localOrigin: { x: number, y: number } }): void {
    const { data, bufferLength, localOrigin } = params;
    this._bufferData = data;
    this._vertexBufferLength = bufferLength;
  ...
  async hitTest(x: number, y: number): Promise<Graphic> {
    if (!this._connection) {
      return;
    }

    // convert the coordinates to map units    
    const mapPoint = this.view.toMap({ x, y });  
      
    const view = this.view;
    return this._connection.invoke("hittest", 
    { 
      ...
      (graphic as any).sourceLayer = this.layer;
      return graphic;
    })
...
#pragma defines      
  ...
  private _connect(): void {
    ...
    connection.invoke("initialize", {
      ...
      const { x, y } = this.view.viewpoint.targetGeometry as Point;
      this._localOrigin.x = x;
      this._localOrigin.y = y;
      connection.invoke("setViewState", { 
        viewpoint: this.view.viewpoint.targetGeometry.toJSON(),
        visibleExtent: this.view.extent.toJSON()
      });

      const customLayer = this.layer as any;
      customLayer.setConnected(true);
    });
  }
  ...
  private _disconnect(): void {
    ...
    const customLayer = this.layer as any;
    customLayer.setConnected(false);

    this._connection.invoke("terminate");
  }
}
